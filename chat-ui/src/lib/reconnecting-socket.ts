...
export interface SocketOptions {
  ...
  onExpired: () => void
  /**
   * onReady fires once a connection is authenticated and subscribed, with
   * whether this is a RECONNECTION rather than the first connection.
   *
   * The distinction is the whole value of it. A push only reaches who is
   * connected, so whatever the server sent while this tab was not, it sent to
   * nobody, and the state those pushes carry has to be re-read when the line
   * comes back. But the FIRST connection has nothing to recover: the page has
   * just read the conversation, and everything after that arrives live. Telling
   * the listener to re-read then is not free, and where a push and a read both
   * mint a token for the same card it is actively harmful.
   */
...
export class ReconnectingSocket {
  ...
  private readonly onReady?: (reconnected: boolean) => void
  // hasConnected distinguishes the first open from every one after it.
